...
b. O programa está a fazer a transposta de uma matriz. A função `operation` recebe uma matriz `m1` e devolve uma nova matriz `m2` com a transposta de `m1`. A função `print_matrix` imprime uma matriz no ecrã.
c. O tamanho máximo que uma matriz pode ter é `MAX_LINHAS` linhas e `MAX_COLUNAS` colunas, que neste caso é 100 linhas e 100 colunas.
